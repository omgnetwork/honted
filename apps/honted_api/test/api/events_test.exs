defmodule HonteD.API.EventsTest do
  ...
  def event_send(receiver) do
    ...
    tx = %HonteD.Transaction.Send{nonce: 0, asset: "asset", amount: 1, from: "from_addr", to: receiver}
  ...
  describe "One can register for events and receive it." do
    test "Subscribe, send event, receive event." do
      ...
      HonteD.Events.notify_committed(e1)
      join()
    end
  ...
  describe "Topics are handled." do
    ...
    test "similar send transactions don't match, but get accepted by Eventer" do
      ...
      unhandled_e = %HonteD.Transaction.Issue{nonce: 0, asset: "asset", amount: 1, dest: address1(), issuer: "issuer_addr"}
      pid1 = client(fn() -> refute_receive(_, @timeout) end)
      new_send_filter(pid1, address1())
      HonteD.Events.notify_committed(unhandled_e)
      join()
    end
    
    test "outgoing send transaction don't match" do
      # NOTE: behavior will require rethinking
      e1 = %HonteD.Transaction.Send{nonce: 0, asset: "asset", amount: 1, from: address1(), to: "to_addr"}
      pid1 = client(fn() -> refute_receive(_, @timeout) end)
      new_send_filter(pid1, address1())
      HonteD.Events.notify_committed(e1)
      join()
